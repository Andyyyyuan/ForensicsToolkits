...
async def list_registered_tools() -> list[ToolMetaResponse]:
    response: list[ToolMetaResponse] = []
...
async def assist_tool_with_ai(payload: ToolAIRequest) -> ToolAIResponse:
    ...
    if tool_id == "log_parser":
        ...
        source, result, reasoning = await ai_analysis_service.analyze_with_meta(
        ...
        return ToolAIResponse(
            tool_id=tool_id,
            mode=payload.mode,
            model=ai_analysis_service.get_model_name(payload.mode) or None,
            source=source,
            reasoning=reasoning,
    ...
    if tool_id == "timestamp_parser":
        result, source, reasoning = await ai_analysis_service.assist_timestamp_with_meta(
    ...
    if tool_id == "encoding_converter":
        result, source, reasoning = await ai_analysis_service.assist_encoding_with_meta(
            payload.user_input.strip(),
            mode=payload.mode,
        )
        return ToolAIResponse(
...
async def assist_tool_with_ai_stream(payload: ToolAIRequest) -> StreamingResponse:
    ...
    if tool_id == "log_parser":
        if not payload.file_id:
            raise HTTPException(status_code=400, detail="日志研判需要 file_id。")
        parsed_result = await _resolve_parsed_log(payload.file_id)
    elif tool_id not in {"timestamp_parser", "hashcat_gui", "encoding_converter", "hash_tool", "sqlite2csv"}:
        raise HTTPException(status_code=400, detail="当前工具未接入统一 AI 辅助。")

    async def event_stream():
        if tool_id == "log_parser":
            iterator = ai_analysis_service.stream_log_analysis(
                parsed_result=parsed_result,
                question=payload.user_input.strip(),
                mode=payload.mode,
            )
        elif tool_id == "timestamp_parser":
            iterator = ai_analysis_service.stream_timestamp_assist(payload.user_input.strip(), mode=payload.mode)
        elif tool_id == "hashcat_gui":
            iterator = ai_analysis_service.stream_hashcat_assist(payload.user_input.strip(), mode=payload.mode)
            ...
            iterator = ai_analysis_service.stream_encoding_assist(payload.user_input.strip(), mode=payload.mode)
...
async def upload_tool_file(tool_id: str = Form(...), file: UploadFile = File(...)) -> FileUploadResponse:
    ...
    try:
        tool_service.get_tool(normalized_tool_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=exc.args[0]) from exc

    try:
        tool_config_service.ensure_enabled(normalized_tool_id)
    except ToolDisabledError as exc:
        raise HTTPException(status_code=403, detail=exc.message) from exc
...
async def inspect_sqlite_database(file_id: str) -> SqliteBrowserResponse:
    ...
    try:
        tool_config_service.ensure_enabled("sqlite2csv")
...
async def run_registered_tool_without_file(tool_id: str, payload: ToolRunRequest) -> ToolRunResponse:
    ...
    try:
        tool_config_service.ensure_enabled(tool_id)
        result = await run_tool(tool_id=tool_id, file_path="", params=payload.params)
    except ToolDisabledError as exc:
        raise HTTPException(status_code=403, detail=exc.message) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

    return ToolRunResponse(tool_id=tool_id, file_id=None, result=result)


@router.post("/timestamp_parser/assist", response_model=TimestampAIAssistResponse)
async def assist_timestamp_parser(payload: ToolAIAssistRequest) -> TimestampAIAssistResponse:
    try:
        tool_config_service.ensure_enabled("timestamp_parser")
    except ToolDisabledError as exc:
        raise HTTPException(status_code=403, detail=exc.message) from exc

    result = await ai_analysis_service.assist_timestamp(payload.user_input.strip())
    return TimestampAIAssistResponse(**result)


@router.post("/hashcat_gui/assist", response_model=HashcatAIAssistResponse)
async def assist_hashcat_gui(payload: ToolAIAssistRequest) -> HashcatAIAssistResponse:
    try:
        tool_config_service.ensure_enabled("hashcat_gui")
    except ToolDisabledError as exc:
        raise HTTPException(status_code=403, detail=exc.message) from exc

    result = await ai_analysis_service.assist_hashcat(payload.user_input.strip())
    return HashcatAIAssistResponse(**result)
